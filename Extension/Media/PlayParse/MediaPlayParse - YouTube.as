...
string RepleaceYouTubeUrl(string url)
{
	...
	if (url.find(YOUTUBE_URL2) >= 0 || url.find(YOUTUBE_URL3) >= 0 || url.find(YOUTUBE_URL4) >= 0 || url.find(YOUTUBE_URL5) >= 0)
	{
		int p = url.rfind("/");
...
//HostOpenConsole();
	...
	if (PlayitemCheck(path))
	{
		...
		tmp_fn.MakeLower();
		if (tmp_fn.find(YOUTUBE_URL2) >= 0 || tmp_fn.find(YOUTUBE_URL3) >= 0 || tmp_fn.find(YOUTUBE_URL4) >= 0 || tmp_fn.find(YOUTUBE_URL5) >= 0) fn = RepleaceYouTubeUrl(fn);
									...
									if (chapterRenderer.isObject())
									{
										...
										if (title.isObject())
										{
											...
											if (simpleText.isString() && timeRangeStartMillis.isInt())
											{
												...
												item["time"] = timeRangeStartMillis.asString();
